<!-- done -->
<title>Giới thiệu</title>
<div id="breadcrumb_background">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="?action=home">Trang chủ</a></li>&nbsp;
            <li class="breadcrumb-item active" aria-current="page"
                style="color: #ffe020; font-weight: 600; font-size: 16px;">&nbsp;Giới thiệu</li>
        </ol>
    </nav>
</div>
<main class="container">
    <h3 id="title_about">GIỚI THIỆU</h3>
    <p>HK Book Storelà một cửa hàng sách trực tuyến nơi bạn có thể
        khám phá và mua sắm các loại sách đa dạng từ các thể loại khác nhau .
    </p>
    <p>
        <strong>Danh Mục Sách Đa Dạng:</strong>
        HK Book Store cung cấp một loạt các thể loại sách bao gồm sách Văn học, Truyẹn tranh comic, sách kinh tế, nghệ
        thuật,
        và nhiều thể loại khác. Điều này giúp đáp ứng đa dạng nhu cầu đọc sách của người đọc.
    </p>
    <p> <strong>Mua Sắm Trực Tuyến:</strong>

        Với HK Book Store, bạn có thể thoải mái mua sắm sách từ nhà mình thông qua trang web của cửa hàng. Quá trình đặt
        hàng và thanh toán được thực hiện một cách thuận tiện, mang lại trải nghiệm mua sắm dễ dàng và nhanh chóng.</p>


    <p> <strong> Trải Nghiệm Mua Sắm Dễ Dàng:</strong>
        Chúng tôi cam kết mang đến cho bạn trải nghiệm mua sắm dễ dàng và thú vị. Từ những cuốn sách mới nhất đến những
        tác phẩm kinh điển, HK Book Store sẽ là đối tác đáng tin cậy của bạn trong hành trình khám phá văn hóa sách. Hãy
        ghé thăm cửa hàng trực tuyến của chúng tôi ngay hôm nay để bắt đầu hành trình đọc sách của bạn cùng HK Book
        Store!
    </p>

</main>